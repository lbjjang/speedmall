...
//표시되는 레이어의 좌측위치
	...
	}else if((960-($(cc[y]).offsetLeft))<$(as[y]).offsetWidth){
		ai=$("sa").offsetWidth - $(as[y]).offsetWidth-7;
	...
	//showBox1.style.top = $("sa").offsetTop+38+"px"; //표시되는 레이어의 메뉴와의 간격
		$(aa[y]).style.color="black";
		for(var i=0;i<7;i++){
			if(i==y){
			var imges = ["-120px -31px","-245px -31px","-366px -31px","-487px -31px","-561px -31px","-690px -31px","-818px -31px"];
			document.getElementById(bb[y]).style.backgroundPosition=imges[y];
		}else{///small/event/event_page20120301_mem.jsp
...
function off(z){//레이어를 숨기는 부분
	var imges = ["-120px 0px","-245px 0px","-366px 0px","-487px 0px","-561px 0px","-690px 0px","-818px 0px"];
	if("A"==z){
		for(var i=0;i<7;i++){
			$(iml[i]).style.display="none";
